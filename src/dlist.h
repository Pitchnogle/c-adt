...
The `destroy` argument provides a way to free dynamically allocated data when `dlist_destroy()` is
called. For example, if the list contains data dynamically allocated using `malloc()`, `destroy` 
should be set to `free` to free the data as the doubly linked-list is destroyed. For a doubly 
linked-list containing data that should not be freed, `destroy` should be set to NULL.
...
Inserts an element just after `element` into the doubly linked-list. When inserting into an emply
list, `element` should point to NULL. The new element contains a pointer to `data`, so the memory\
referenced by `data` should remain valid as long as the element remains in the list.
...
*/
int dlist_insert_next(DList *list, DList_Element *element, const void *data);

/**
Function to insert an element into a doubly linked-list

Inserts an element just before `element` into the doubly linked-list. When inserting into an empty
list, `element` should point to NULL. The new element contains a pointer to `data`, so the memory
referenced by `data` should remain valid as long as the element remains in the list.
...
@return 0 if inserting into list was successful, otherwise -1
*/
int dlist_insert_prev(DList *list, DList_Element *element, const void *data);

/**
Function to remove an element from a doubly linked list

Removes the element specified as `element` from the doubly linked-list. Upon return `data` points
to the data stored in the element that was removed.
...
@return 0 if inserting into list was successful, otherwise -1
*/
int dlist_remove(DList *list, DList_Element *element, void **data);
