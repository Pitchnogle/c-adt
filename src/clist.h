...
The `destroy` argument provides a way to free dynamically allocated data when `list_destroy()` is
called. For example, if the list contains data dynamically allocated using `malloc`, `destroy` 
should be set to `free` to free the data as the linked-list is destroyed. For a circular linked-list
...
Inserts an element just after `element` into the circular linked-list. If `element` is NULL, the
new element is inserted at the head of the list. The new element contains a pointer to `data`, so
...
*/
int clist_insert_next(CList *list, CList_Element *element, const void *data);

/**
Function to remove an element from a circular linked list

Removes the element just past `element` from the circular linked-list. Upon return `data` points
...
@return 0 if inserting into list was successful, otherwise -1
*/
int clist_remove_next(CList *list, CList_Element *element, void **data);
