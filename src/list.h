...
@pre
Must be called before the list can be used by any other operation
...
Inserts an element just after `element` into the linked-list. If `element` is NULL, the new element
is inserted at the head of the list. The new element contains a pointer to `data`, so the memory
...
Removes the element just past `element` from the linked-list. If `element` is NULL, the element at
the head of the list is removed. Upon return `data` points to the data stored in the element that
was removed.
